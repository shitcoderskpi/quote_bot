...
cppcoro::task<> template_storage::load_template_async(const std::filesystem::path &path,
...
cppcoro::task<> template_storage::load_templates_async(const std::filesystem::path &path,
                                                            cppcoro::static_thread_pool &pool) {
    if (!std::filesystem::exists(path) || !std::filesystem::is_directory(path)) {
        logger->error("Template directory {} does not exist, or is not a directory", path.string());
        co_return;
